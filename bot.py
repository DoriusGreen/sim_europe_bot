...
# === СТАНДАРТНИЙ ЗАПИТ ДАНИХ ДЛЯ ОФОРМЛЕННЯ (ЖОРСТКИЙ ШАБЛОН) ===
ORDER_INFO_REQUEST = (
...
def build_system_prompt() -> str:
    ...
    return (
        ...
        "СТАНДАРТНИЙ ЗАПИТ ДАНИХ (використовуй ДОСЛІВНО, без будь-якого додаткового тексту до/після):\n"
        "🛒 Для оформлення замовлення напишіть:\n\n"
        "1. Ім'я та прізвище.\n"
        "2. Номер телефону.\n"
        "3. Місто та № відділення \"Нової Пошти\".\n"
        "4. Країна(и) та кількість sim-карт.\n\n"
        "Правило: якщо бракує ХОЧА Б ОДНОГО з цих чотирьох пунктів — відповідай ТІЛЬКИ цим шаблоном. "
        "НЕ перефразовуй і не додавай нічого зайвого. Коли всі дані є — показуй підсумок і додай тег <SEND_THANK_YOU> на новому рядку.\n\n"
        ...
        "• Якщо даних бракує — став ОДНЕ повідомлення зі списком пунктів, яких не вистачає (але якщо бракує щось із 4 обовʼязкових — використай саме шаблон вище).\n"
        "• Якщо клієнт пише «потрібна Англія 2 шт» — не перепитуй очевидне. Попроси ТІЛЬКИ відсутні дані пакетом (через шаблон, якщо потрібно).\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- ПІДСТРАХОВКА ШАБЛОНУ: якщо модель просить дані нестандартно, віддаємо саме твій шаблон ---
    # Вважаємо, що модель «просить дані», якщо згадано ключові пункти,
    # але немає точного блоку ORDER_INFO_REQUEST.
    lower = reply_text.lower()
    asks_for_fields = any(k in lower for k in ["ім'я", "прізвище", "номер телефону", "місто", "поштомат", "нова пошта", "країна", "sim"])
    if asks_for_fields and ORDER_INFO_REQUEST not in reply_text:
        reply_text = ORDER_INFO_REQUEST
