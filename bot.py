...
def _parse_ids(env: Optional[str]) -> Set[int]:
    [cite_start]out: Set[int] = set() [cite: 1-2]
    if not env:
        return out
    for x in env.split(","):
        x = x.strip()
...
def _is_manager_message(msg: Message) -> bool:
    [cite_start]u = msg.from_user [cite: 4]
    if not u:
        return False
...
ORDER_INFO_REQUEST = (
    ...
    [cite_start]"1. Ім'я та прізвище.\n" [cite: 4-5]
    [cite_start]"2. Номер телефону.\n" [cite: 5]
    [cite_start]"3. Місто та № відділення \"Нової Пошти\".\n" [cite: 6]
    [cite_start]"4. Країна(и) та кількість sim-карт." [cite: 7]
)
...
DIAL_CODES = {
    ...
    "НІМЕЧЧИНА": "+49",
    "НІДЕРЛАНДИ": "+31",
    [cite_start]"ІТАЛІЯ": "+39", [cite: 10-11]
    "ЧЕХІЯ": "+420",
    "МОЛДОВА": "+373",
    "КАЗАХСТАН": "+7",
    "ЛАТВІЯ": "+371",
...
def normalize_country(name: str) -> str:
    ...
    if n in ("USA", "U.S.A.", "UNITED STATES", "UNITED STATES OF AMERICA", "ШТАТИ", "АМЕРИКА", "US", "U.S."):
        return "США"
    [cite_start]if n in ("ITALY","ИТАЛИЯ","ІТАЛІЯ","ITALIA","+39"): [cite: 11-12]
        return "ІТАЛІЯ"
...
def canonical_operator(op: Optional[str]) -> Optional[str]:
    ...
    if o in ("lebara", "лебара"):
        return "Lebara"
    [cite_start]if o in ("vodafone", "водафон", "водофон"): [cite: 13]
        return "Vodafone"
    if o in ("three", "трі", "3"):
        return "Three"
    return None
...
def canonical_operator_any(op: Optional[str]) -> Optional[str]:
    ...
    mapping = {
        ...
        [cite_start]"Vodafone": ["vodafone","водафон","водофон"], [cite: 13-14]
...
def unit_price(country_norm: str, qty: int) -> Optional[int]:
    ...
    if not tiers:
        [cite_start]return None [cite: 14-15]
...
def render_price_block(country_key: str) -> str:
    ...
    [cite_start]tiers = PRICE_TIERS.get(country_key, []) [cite: 16]
    if not tiers:
    ...
    for idx, (min_q, price) in enumerate(tiers_sorted):
        ...
        [cite_start]if not inserted_gap and min_q >= 100 and idx > 0: [cite: 16-17]
            lines.append("")
            inserted_gap = True
...
def available_list_text() -> str:
    ...
    [cite_start]if len(names) == 1: return names[0] [cite: 18]
...
def _smart_title(s: str) -> str:
    ...
    out = []
    for p in parts:
        [cite_start]sub = "-".join(_cap_word(x) for x in p.split("-")) [cite: 19-20]
        out.append(sub)
    return " ".join(out)
...
def format_phone(phone: str) -> str:
    ...
    if len(digits) == 12 and digits.startswith("380"):
        [cite_start]digits = "0" + digits[3:] [cite: 20-21]
    if len(digits) == 10:
...
[cite_start]ORDER_LINE = "{flag} {disp}, {qty} шт — {line_total} грн   \n" [cite: 21-22]
...
def render_order(order: OrderData) -> str:
    lines = []
    [cite_start]grand_total = 0 [cite: 22-23]
    counted_countries = 0
    ...
    for it in order.items:
        ...
        [cite_start]price = unit_price(c_norm, it.qty) [cite: 23-24]
        ...
        [cite_start]lines.append(ORDER_LINE.format( [cite: 25]
            flag=flag, disp=disp, qty=it.qty, line_total=line_total_str
        ))

    address_line = ""
    # Перевірка, чи є 'np' порожнім. Якщо так, це адресна доставка.
    if not order.np or order.np.isspace():
        address_line = f"📫: {format_city(order.city)}"
    else:
...
# ==== JSON парсери ====
[cite_start]ORDER_JSON_RE = re.compile(r"\{[\s\S]*\}") [cite: 25-26]
...
def try_parse_order_json(text: str) -> Optional[OrderData]:
    ...
    if not m:
        return None
    try:
        [cite_start]data = json.loads(m.group(0)) [cite: 27]
        items = [OrderItem(
            country=i["country"],
            qty=int(i["qty"]),
        ...
        return OrderData(
            ...
            [cite_start]city=data.get("city", "").strip(), [cite: 27-28]
...
def try_parse_ussd_json(text: str) -> Optional[List[Dict[str, str]]]:
    ...
    if not m:
        return None
    try:
        [cite_start]data = json.loads(m.group(0)) [cite: 29-30]
        if data.get("ask_ussd") is True and isinstance(data.get("targets"), list):
            out = []
            for t in data["targets"]:
                if not isinstance(t, dict):
                    continue
                [cite_start]c = t.get("country") [cite: 31]
                o = t.get("operator")
                if c:
        ...
        [cite_start]return None [cite: 31-32]
...
US_ACTIVATION_MSG = (
    [cite_start]"США, на відміну від інших країн, потребують поповнення для активації. Після поповнення SIM працюватиме на прийом SMS.\n\n" [cite: 32-33]
    "Як активувати та поповнити сім-карту США?\n\n"
    "https://www.lycamobile.us/en/activate-sim\n"
    [cite_start]"1. На цьому сайті вводите дані сімки для її попередньої активації.\n" [cite: 33-34]
    "Отриманий на сайті номер сім-карти записуєте.\n\n"
    "https://www.lycamobile.us/en/quick-top-up/\n"
    [cite_start]"2. Далі, ось тут, вказавши номер, отриманий на попередньому сайті, поповнюєте сім-карту, після поповнення (мінімум на 23$) вона стане активною та буде приймати SMS." [cite: 34-35]
)
...
USSD_DATA: Dict[str, List[Tuple[Optional[str], str]]] = {
    ...
    [cite_start]"НІДЕРЛАНДИ": [("Lycamobile", "*102#")], [cite: 35-36]
...
def render_ussd_targets(targets: List[Dict[str, str]]) -> str:
    ...
    for t in targets:
        ...
        if not country:
            [cite_start]continue [cite: 36-37]
        ...
        if op_req and pairs:
            [cite_start]pairs = [p for p in pairs if (p[0] and canonical_operator_any(p[0]) == op_req)] [cite: 37-38]
        if not pairs:
            if op_req:
            ...
            result_lines.append("")
            [cite_start]continue [cite: 38-39]
    ...
    while result_lines and result_lines[-1] == "":
        [cite_start]result_lines.pop() [cite: 39-40]
...
def detect_qty_only(text: str) -> Optional[int]:
    if not text:
        [cite_start]return None [cite: 40-41]
...
COUNTRY_KEYWORDS: Dict[str, List[str]] = {
    ...
    [cite_start]"НІМЕЧЧИНА": ["німеч", "герман", "german", "+49", "deutsch"], [cite: 41-42]
...
def _country_mentions_with_pos(text: str) -> List[Tuple[str, int]]:
    ...
    [cite_start]for key, subs in COUNTRY_KEYWORDS.items(): [cite: 42-43]
        best = None
        for s in subs:
            i = low.find(s)
            if i != -1:
        ...
        if best is not None:
            [cite_start]found.append((key, best)) [cite: 43-44]
...
def detect_point4_items(text: str) -> List[Tuple[str, int]]:
    ...
    [cite_start]items: List[Tuple[str, int]] = [] [cite: 44-45]
    ...
    for ci, (country, cpos) in enumerate(mentions):
        ...
        for ni, (nstr, npos) in enumerate(nums):
            if ni in used_num_idx:
                continue
            [cite_start]dist = abs(npos - cpos) [cite: 45-46]
        ...
        if best_idx is not None:
            ...
            [cite_start]used_num_idx.add(best_idx) [cite: 46-47]
    ...
    if len(items) == 0 or len(items) < len(mentions):
        ...
        if m:
            ...
            for ci, (country, _) in enumerate(mentions):
                [cite_start]if ci not in used_country_idx: [cite: 47-48]
...
def build_system_prompt() -> str:
    return (
         # === РОЛЬ ТА КОНТЕКСТ ===
        [cite_start]"Ти — дружелюбний і корисний Telegram-бот в інтернет-магазині SIM-карт. Чітко та суворо дотримуйся прописаних інструкцій, якщо щось не зрозуміло, то не вигадуй, а краще перепитай клієнта що він мав на увазі.\n" [cite: 48-49]
        [cite_start]"На початку чату клієнт уже отримує від акаунта власника перелік країн у наявності, цін на сім-карти цих країн та інформацію для доставки — ти це НЕ ДУБЛЮЄШ. Уважно все перевіряєш, якщо клієнт запитав про ціни, а ти бачиш, що перелік цін був в одному з останніх трьох повідомлень, то просто вказуєш, що перелік цін вище. [cite: 49-50] [cite_start]Якщо перелік десь далеко, то надаєш клієнту перелік цін.\n" [cite: 51]
        ...
        [cite_start]"Завжди аналізуй кожне повідомлення на наявність елементів замовлення (пункти 1-4 нижче). Якщо в повідомленні або історії є хоча б один пункт, починай процес збору даних: показуй, що бракує, або формуй JSON, якщо все є.\n" [cite: 51-52]
        ...
        [cite_start]"ПОВНЕ замовлення складається з 4 пунктів:\n" [cite: 53]
        [cite_start]"1. Ім'я та прізвище (Не плутай ім'я клієнта з по-батькові! Записуй лише імя та прізвище.).\n" [cite: 53-54]
        [cite_start]"2. Номер телефону.\n" [cite: 55]
        [cite_start]"3. Місто та № відділення «Нової Пошти».\n" [cite: 56]
        [cite_start]"4. Країна(и) та кількість sim-карт.\n\n" [cite: 57]
        "Особливий випадок — адресна доставка. Якщо клієнт чітко вказує на кур'єрську або адресну доставку (наприклад, 'доставка за адресою', 'кур'єром нової пошти'), то в полі `city` JSON-об'єкта потрібно вказати всю адресу як є, а поле `np` залишити порожнім (`\"np\": \"\"`).\n\n"
        ...
        [cite_start]"Якщо виявлено хоча б один пункт, але ще НЕ вказано ВСІ 4 — відповідай СУВОРО в такому вигляді (без зайвого тексту до/після):\n" [cite: 58]
        "📝 Залишилось вказати:\n\n"
        ...
        "{\n"
        '  "ask_prices": true,\n'
        '  "countries": ["ALL" або перелік ключів, напр. [cite_start]"ВЕЛИКОБРИТАНІЯ","США"]\n' [cite: 61-62]
        "}\n\n"
        ...
        [cite_start]"Для прайсу/наявності доступні ЛИШЕ: ВЕЛИКОБРИТАНІЯ, НІДЕРЛАНДИ, НІМЕЧЧИНА, ФРАНЦІЯ, ІСПАНІЯ, ЧЕХІЯ, ПОЛЬЩА, ЛИТВА, ЛАТВІЯ, КАЗАХСТАН, МАРОККО, США.\n" [cite: 63]
        "Не стверджуй наявність/ціну для інших країн (але довідку USSD можна давати і для інших, якщо відома комбінація).\n\n"

        # === СЕМАНТИКА ===
        [cite_start]"• Розумій країни за синонімами/містами/мовою (UK/United Kingdom/+44/Британія → ВЕЛИКОБРИТАНІЯ; USA/Америка/Штати → США).\n" [cite: 63-64]
        ...
        [cite_start]"• Якщо клієнт для Англії називає оператора (O2, Lebara, Vodafone) — додай поле \"operator\" з канонічним значенням; інакше — не додавай це поле.\n" [cite: 64-65]
        "• Текстові кількості (пара/десяток/кілька) перетворюй у число або попроси уточнення через пункт 4.\n\n"

        # === ЕСКАЛАЦІЯ ДО ЛЮДИНИ ===
        [cite_start]"Запити «зв’язатися з людиною/менеджером/оператором» — це звернення до МЕНЕДЖЕРА магазину. Відповідай: «Очікуйте відповіді менеджера.»\n" [cite: 65-66]
        ...
        [cite_start]"Відповідай, що в нас є такі номери, але лише по країні Англія. Коштують вони від 1500 грн до 2500 грн. Якщо цікаво, то менеджер надішле вам підбірку.\n\n" [cite: 66-67]

         # === Проблема з приймомо SMS ===
        [cite_start]"Іноді клієнти пишуть, що сім-карта не працює, або їм не приходить СМС. Ось яку інформацію в цьому випадку ти маєш їм відправити:\n\n" [cite: 67-68]
        [cite_start]"1. Перевірте чи підключена сім-карта до мережі. Якщо ні, спробуйте підключитись через налаштування в телефоні вручну (просто виберіть будь-яку мережу із запропонованих).\n\n" [cite: 69]
        [cite_start]"2. Якщо мережа є, але СМС не приходить, то це точно проблема на стороні сервісу, і потрібно спробувати пізніше, бажано через 24 години.\n\n" [cite: 70]
        [cite_start]"3. Щоб переконатись в тому, що сім-карта точно приймає СМС, можете спробувати відправити повідомлення з українського номеру.\n\n" [cite: 71]
        ...
        [cite_start]"Чи зможу я зареєструвати месенджери?\n" [cite: 72]
        [cite_start]"Так! Ви одразу зможете зареєструвати WhatsApp, Telegram, Viber та інші, а також прийняти SMS.\n\n" [cite: 72-73]
        "Чи потрібно поповнювати?\n"
        [cite_start]"Сім-карта одразу працює на прийом SMS, поповнення потрібне лише для вхідних та вихідних дзвінків. Поповнити можна на сайті ding.com, використавши платіжку систему PayPal.\n\n" [cite: 73-74]
        "Скільки SIM-карта буде активна?\n"
        [cite_start]"Зазвичай до півроку; щоб працювала довше — раз на 6 міс поповнюйте на 10 фунтів/євро.\n\n" [cite: 74-75]
        "Які тарифи?\n"
        ...
        [cite_start]"Чи вже відправили? Чи є ТТН/трек-номер?\n" [cite: 76-77]
        [cite_start]"Зазвичай відправляємо в день замовлення або протягом 24 годин. Якщо потрібна ТТН — очікуйте відповіді менеджера.\n\n" [cite: 77-78]
        "Як оплатити?\n"
        [cite_start]"Зазвичай накладений платіж. За бажанням — карта або USDT (TRC-20).\n\n" [cite: 78-79]
        "Чи можлива відправка в інші країни?\n"
        "Так, від 3 шт, повна передоплата, «Нова Пошта».\n\n"
        "Який оператор для конкретної країни?\n"
        [cite_start]"Ти не пропонуєш операторів сам, тільки відповідаєш, коли клієнт сам конкретно запитує про оператора для країни. Якщо клієнт запитує про неіснуючого оператора для країни (наприклад, Vodafone для Німеччини, але є тільки Lebara), скажи, що в роботі оператор Lebara (або той, що є) нічим не відрізняється. [cite: 79-80] Оператори для країн: Нідерланди - Lebara; Франція - Lebara; Іспанія - Lebara; [cite_start]Чехія - T-Mobile та Kaktus; [cite: 81] Польща - Play (але потрібно уточнити у менеджера); Литва - Labas; [cite_start]Казахстан - Tele2; [cite: 82] [cite_start]США - Lycamobile.\n\n" [cite: 83]

        # === США — ОСОБЛИВО ===
        [cite_start]"США — на відміну від інших, потребують поповнення для активації. Після поповнення SIM працюватиме на прийом SMS.\n\n" [cite: 83-84]
...
def build_followup_prompt() -> str:
    return (
        ...
        [cite_start]"Якщо просили ЛИШЕ ціну/прайс — поверни порожній рядок. Не пиши шаблонні підтвердження наявності.\n\n" [cite: 84-85]
...
def is_ack_only(text: str) -> bool:
    if not text:
        return False
    [cite_start]low = text.strip().lower() [cite: 87]
...
def is_meaningful_followup(text: str) -> bool:
    ...
    if not t:
        return False
    low = t.lower()
    [cite_start]banned_words = ["ціни", "прайс", "надіслано", "див. вище", "вище", "повторю"] [cite: 87-88]
...
# [cite_start]======== GPT-парсер для повідомлень менеджера ======== [cite: 89-90]
...
def build_manager_parser_prompt() -> str:
    ...
    return (
        [cite_start]"Ти — сервіс для вилучення даних. Твоє завдання — розібрати неструктурований текст із даними замовлення та повернути їх у вигляді чіткого JSON-об'єкта.\n\n" [cite: 90-91]
        ...
        [cite_start]"- Перелік замовлених SIM-карт (країна та кількість).\n" [cite: 91-92]
        [cite_start]"- Назву оператора (напр. Three, Lebara, O2, Vodafone), зазвичай для Великобританії. Можуть вказуватись кирилицею, чи скорочено.\n" [cite: 92-93]
        "- Сторонні коментарі, які потрібно ігнорувати.\n\n"
        "Правила:\n"
        [cite_start]"1. Твоя відповідь має бути ТІЛЬКИ JSON-об'єктом. Без жодних пояснень, тексту до чи після, чи markdown-форматування.\n" [cite: 93-94]
        [cite_start]"2. Для країн використовуй СУВОРО канонічні назви з цього списку: " + country_keys + ".\n" [cite: 95]
        [cite_start]"3. Якщо для країни ВЕЛИКОБРИТАНІЯ (Англія) вказаний оператор (напр. O2, Lebara, Vodafone, Three), додай в об'єкт товару поле `\"operator\"`. [cite: 96] Використовуй канонічне значення: `\"O2\"`, `\"Lebara\"`, `\"Vodafone\"`, `\"Three\"`. [cite_start]Якщо оператор не вказаний, не додавай це поле.\n" [cite: 97]
        [cite_start]"4. Поля `full_name`, `phone`, `city`, `np` мають бути рядками. Поле `items` — масивом об'єктів.\n" [cite: 98]
        [cite_start]"5. Якщо якісь дані відсутні в тексті, залиш відповідне поле як порожній рядок \"\" або порожній масив [].\n\n" [cite: 99]
        "Приклад:\n"
        [cite_start]"Вхідний текст: 'Так, це новий клієнт, Іван Франко, тел 0991234567. Хоче 2 сімки для Англії оператора водафон та 1 для США. Відправка в Київ, відділення 30. Каже, що оплатить на карту.'\n" [cite: 99-100]
        ...
        '  "np": "30",\n'
        '  "items": [\n'
        [cite_start]'    {"country": "ВЕЛИКОБРИТАНІЯ", "qty": 2, "operator": "Vodafone"},\n' [cite: 100-101]
        '    {"country": "США", "qty": 1}\n'
        '  ]\n'
        "}"
    )
...
async def _ask_gpt_to_parse_manager_order(text: str) -> str:
    ...
    try:
        [cite_start]response = await openai.ChatCompletion.acreate( [cite: 102]
            model="gpt-4o",
            messages=messages,
            max_tokens=500,
            temperature=0.1,
        ...
        [cite_start]logger.error(f"Помилка GPT-парсера для менеджера: {e}") [cite: 102-103]
        return ""
...
def try_parse_manager_order_json(json_text: str) -> Optional[OrderData]:
    ...
    try:
        ...
        if not all(k in data for k in ["full_name", "phone", "city", "np", "items"]):
            ...
            [cite_start]return None [cite: 103-104]
        items = [OrderItem(
            country=i["country"],
            qty=int(i["qty"]),
            operator=i.get("operator")
        ...
        if not data.get("full_name") and not data.get("phone") and not items:
            ...
            [cite_start]return None [cite: 104-105]
        return OrderData(
            full_name=data.get("full_name", "").strip(),
            phone=data.get("phone", "").strip(),
    ...
    [cite_start]except (json.JSONDecodeError, TypeError, KeyError, ValueError) as e: [cite: 105-106]
...
def render_order_for_group(order: OrderData, paid: bool) -> str:
    ...
    for it in order.items:
        ...
        op_suf = f" (оператор {op})" if (op and c_norm == "ВЕЛИКОБРИТАНІЯ") else ""
        disp = disp_base + op_suf
        [cite_start]flag = FLAGS.get(c_norm, "") [cite: 107-108]
        if paid:
            ...
            if price is None:
                [cite_start]line_total_str = "договірна" [cite: 108-109]
                line = f"{flag} {disp}, {it.qty} шт — {line_total_str}  \n"
            else:
                line_total = price * it.qty
                grand_total += line_total
                counted += 1
                [cite_start]line = f"{flag} {disp}, {it.qty} шт — {line_total} грн  \n" [cite: 109-110]
        lines.append(line)

    address_line = ""
    # Аналогічна перевірка для адресної доставки.
    if not order.np or order.np.isspace():
        address_line = f"📫: {format_city(order.city)}"
    else:
        address_line = f"{format_city(order.city)} № {format_np(order.np)}"
    ...
    [cite_start]return header + "".join(lines) + footer [cite: 110-111]
...
async def _openai_chat(messages: List[Dict[str, str]]) -> str:
    try:
        ...
        return response.choices[0].message["content"]
    [cite_start]except Exception as e: [cite: 111-112]
        logger.error(f"Помилка при зверненні до OpenAI: {e}")
        [cite_start]return "Вибачте, сталася технічна помилка. Спробуйте, будь ласка, ще раз." [cite: 112-113]
...
async def _ask_gpt_followup(history: List[Dict[str, str]], user_payload: str) -> str:
    ...
    try:
        response = await openai.ChatCompletion.acreate(
            [cite_start]model="gpt-4o", [cite: 113-114]
            messages=messages,
            max_tokens=300,
            temperature=0.2,
        )
...
async def _ask_gpt_force_point4(history: List[Dict[str, str]], user_payload: str) -> str:
    [cite_start]messages = [{"role": "system", "content": build_force_point4_prompt()}] [cite: 114-115]
    messages.extend(history)
    messages.append({"role": "user", "content": user_payload})
    try:
        response = await openai.ChatCompletion.acreate(
        ...
        return response.choices[0].message["content"].strip()
    [cite_start]except Exception as e: [cite: 115-116]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        [cite_start]"Вітаю! Я допоможу вам оформити замовлення на SIM-карти, а також постараюсь надати відповіді на всі ваші запитання." [cite: 116-117]
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Видалено фільтр is_ack_only. [cite_start]Всі повідомлення йдуть на обробку. [cite: 117-118]
    ...
    if (
        ...
        if msg.reply_to_message:
            ...
            [cite_start]operator_reply = canonical_operator(raw_user_message) [cite: 118-119]
            ...
            # Сценарій 2: Менеджер відповів назвою оператора
                ...
                [cite_start]modified_lines = [] [cite: 123]
                found_uk = False
                for line in original_text.splitlines():
                    if ("Англія" in line or "ВЕЛИКОБРИТАНІЯ" in line) and "оператор" not in line:
                        # Вставляємо оператора після назви країни
                        [cite_start]line = line.replace(",", f" (оператор {operator_reply}),", 1) [cite: 124]
                        found_uk = True
                    modified_lines.append(line)
                
                [cite_start]if found_uk: [cite: 125]
                    final_text = "\n".join(modified_lines)
                    try:
                        await context.bot.delete_message(chat_id=msg.chat.id, message_id=original_msg_id)
                        [cite_start]await context.bot.delete_message(chat_id=msg.chat.id, message_id=msg.message_id) [cite: 125-126]
                    except Exception as e:
                        logger.warning(f"Не вдалося видалити повідомлення при додаванні оператора: {e}")
                    await context.bot.send_message(chat_id=msg.chat.id, text=final_text)
                    [cite_start]return [cite: 126-127]
        ...
        if len(parts) > 1:
            [cite_start]text_for_gpt = parts[0] [cite: 127-128]
        ...
        if parsed_order:
            ...
            try:
                await context.bot.delete_message(chat_id=msg.chat.id, message_id=msg.message_id)
            [cite_start]except Exception as e: [cite: 128-129]
            ...
            [cite_start]return [cite: 129-130]
        # --- КІНЕЦЬ ЗМІН ---
        else:
    ...
    if _is_manager_message(msg):
        ...
        [cite_start]if text: [cite: 130-131]
    ...
    if qty_only and last_price_countries:
        ...
        user_payload += (
            [cite_start]f"\n\n[ПІДСКАЗКА ДЛЯ ПАРСИНГУ: користувач вказав лише кількість {qty_only} шт. " [cite: 132-133]
            f"Застосуй її до кожної з останніх країн, для яких щойно показували прайс: {hint_countries}. "
            [cite_start]f"Це відповідає пункту 4 (країна/кількість).]" [cite: 133-134]
        )
    ...
    if p4_items:
        [cite_start]pairs_text = "; ".join(f"{DISPLAY.get(c, c.title())} — {q}" for c, q in p4_items) [cite: 134-135]
        user_payload += (
    ...
    [cite_start]if context.chat_data.get("awaiting_missing") == {1, 2, 3} and context.chat_data.get("point4_hint"): [cite: 136]
        h = context.chat_data["point4_hint"]
        if "qty" in h and "countries" in h:
            hc = ", ".join(h["countries"])
            user_payload += (
                f"\n\n[НАГАДУВАННЯ ДЛЯ ПАРСИНГУ: пункт 4 вже відомий: {hc} — по {h['qty']} шт. "
                [cite_start]f"Додай/склей із пунктами 1–3.]" [cite: 136-137]
            )
        elif "items" in h:
            [cite_start]pairs_text = "; ".join(f"{DISPLAY.get(c, c.title())} — {q}" for c, q in h["items"]) [cite: 137-138]
            user_payload += (
    ...
    if awaiting == {4}:
        [cite_start]force_json = await _ask_gpt_force_point4(history, user_payload) [cite: 138-139]
        ...
        if forced and forced.items and all([forced.full_name, forced.phone, forced.city, forced.np]):
            ...
            [cite_start]context.chat_data.pop("point4_hint", None) [cite: 139-140]
            history.append({"role": "user", "content": raw_user_message})
            history.append({"role": "assistant", "content": summary})
            _prune_history(history)
            await msg.reply_text(summary)
            [cite_start]await msg.reply_text("Дякуємо за замовлення, воно буде відправлено протягом 24 годин. 😊") [cite: 140-141]
            try:
                username = update.effective_user.username
                forward_text = f"@{username}\n{summary}" if username else summary
                await context.bot.send_message(
                    chat_id=ORDER_FORWARD_CHAT_ID,
                    [cite_start]text=forward_text [cite: 141-142]
                )
            except Exception as e:
    ...
    [cite_start]if "Залишилось вказати:" in reply_text and "📝 Залишилось вказати:" not in reply_text: [cite: 142-143]
    ...
    if parsed and parsed.items and all([parsed.full_name, parsed.phone, parsed.city, parsed.np]):
        ...
        [cite_start]last_time = context.chat_data.get("last_order_time", 0) [cite: 144-145]
        if last_sig and current_sig == last_sig and (time.time() - last_time <= ORDER_DUP_WINDOW_SEC):
            ...
            context.chat_data.pop("awaiting_missing", None)
            [cite_start]context.chat_data.pop("point4_hint", None) [cite: 145-146]
            return
        ...
        history.append({"role": "assistant", "content": summary})
        _prune_history(history)
        [cite_start]await msg.reply_text(summary) [cite: 146-147]
        [cite_start]await msg.reply_text("Дякуємо за замовлення, воно буде відправлено протягом 24 годин. 😊") [cite: 147-148]
        try:
            username = update.effective_user.username
            forward_text = f"@{username}\n{summary}" if username else summary
            await context.bot.send_message(
        ...
        [cite_start]except Exception as e: [cite: 148-149]
            logger.warning(f"Не вдалося надіслати замовлення в групу: {e}")
        return
    ...
    if price_countries is not None:
        ...
        [cite_start]invalid = [price_countries[i] for i, k in enumerate(normalized) [cite: 149-150]
        ...
        if want_all:
            [cite_start]price_msg = "".join(render_price_block(k) for k in PRICE_TIERS.keys()) [cite: 151]
            history.append({"role": "user", "content": raw_user_message})
            ...
            [cite_start]history.append({"role": "user", "content": raw_user_message}) [cite: 151-152]
            history.append({"role": "assistant", "content": price_msg})
            _prune_history(history)
            await msg.reply_text(price_msg)
            if invalid:
            ...
            [cite_start]unavailable_msg = render_unavailable(invalid if invalid else price_countries) [cite: 152-153]
            history.append({"role": "user", "content": raw_user_message})
        ...
        if ussd_targets_followup:
            [cite_start]formatted = render_ussd_targets(ussd_targets_followup) or FALLBACK_PLASTIC_MSG [cite: 153-154]
            ...
            return
        [cite_start]if is_meaningful_followup(follow): [cite: 155]
    ...
    if ussd_targets:
        [cite_start]formatted = render_ussd_targets(ussd_targets) or FALLBACK_PLASTIC_MSG [cite: 156]
        history.append({"role": "user", "content": raw_user_message})
        history.append({"role": "assistant", "content": formatted})
        _prune_history(history)
        context.chat_data.pop("awaiting_missing", None)
    ...
    [cite_start]elif missing: [cite: 156-157]
    ...
    if reply_text:
        ...
        _prune_history(history)
        [cite_start]await msg.reply_text(reply_text) [cite: 158]
    else:
        [cite_start]logger.info(f"GPT повернув пусту відповідь на повідомлення: '{raw_user_message}'. Відповідь не надіслано.") [cite: 158-159]
...
def main():
    ...
    application.run_webhook(
        ...
        port=PORT,
        url_path="",
        [cite_start]webhook_url=WEBHOOK_URL [cite: 159-160]
    )
