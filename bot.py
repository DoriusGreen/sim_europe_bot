...
def _order_signature(order: OrderData) -> str:
    ...
    # --- ПОЧАТОК ЗМІН ---
    return f"{format_full_name(order.full_name)}|{format_phone(order.phone)}|{order.city.strip()}|{str(order.region or '').strip()}|{str(order.np or '').strip()}|{items_sig}"
    # --- КІНЕЦЬ ЗМІН ---
...
def render_order(order: OrderData) -> str:
    ...
    delivery_line = ""
    # --- ПОЧАТОК ЗМІН ---
    if order.city and order.np:
        # Стандартна доставка на відділення
        city_part = format_city(order.city)
        region_part = f" ({order.region.strip()})" if order.region else ""
        np_part = f" № {format_np(order.np)}"
        delivery_line = f"{city_part}{region_part}{np_part}"
    elif order.city:
        # Адресна доставка (поле np пусте)
        delivery_line = f"📫: {order.city.strip()}"
    # --- КІНЕЦЬ ЗМІН ---

    header_parts = [
        format_full_name(order.full_name),
        format_phone(order.phone),
    ]
    if delivery_line:
        header_parts.append(delivery_line)
...
def try_parse_order_json(text: str) -> Optional[OrderData]:
    ...
    try:
        ...
        return OrderData(
            ...
            items=items,
            region=data.get("region") # Отримуємо нове поле
        )
        # --- КІНЕЦЬ ЗМІН ---
...
def build_system_prompt() -> str:
    return (
        ...
        "Уважно все перевіряєш, якщо клієнт запитав про ціни, а ти бачиш, що перелік цін був в одному з останніх трьох повідомлень, то просто вказуєш, що перелік цін вище. Якщо перелік десь далеко, то надаєш клієнту перелік цін.\n"
        ...
        # --- ПОЧАТОК ЗМІН ---
        "=== ПРАВИЛА ОБРОБКИ ДОСТАВКИ ===\n"
        "1. **Кур'єрська доставка:** Розпізнавай її ТІЛЬКИ ЯКЩО клієнт прямо написав ключові слова: «кур'єр», «адресна», «доставка на адресу», «додому». Тільки за наявності цих слів записуй повну адресу в поле `city`, а поле `np` залишай порожнім.\n"
        "2. **Доставка на відділення:** В усіх інших випадках доставка йде на відділення «Нової Пошти». Якщо клієнт вказує область (напр. 'Гребінка Полтавської обл.'), витягуй її в окреме поле `region`. Наприклад: `\"city\": \"Гребінка\"`, `\"region\": \"Полтавська обл.\"`. Якщо область не вказана, поле `region` має бути відсутнім або `null`.\n"
        "3. **СУВОРА ЗАБОРОНА:** Якщо в повідомленні НЕМАЄ ключових слів для кур'єрської доставки, то поле `np` (номер відділення) **ОБОВ'ЯЗКОВО** повинно бути заповнене. Якщо ти не можеш знайти номер відділення в тексті (наприклад, клієнт вказав тільки вулицю), **ЗАБОРОНЕНО** заповнювати поле `city` цією адресою. В такому випадку ЗАЛИШ ОБИДВА ПОЛЯ, `city` та `np`, ПОРОЖНІМИ (`\"\"`), щоб бот міг перепитати клієнта.\n\n"
        # --- КІНЕЦЬ ЗМІН ---
        ...
        '  "region": "Область (якщо вказана, інакше null)",\n'
        '  "np": "Номер відділення (якщо доставка не кур\'єрська)",\n'
        ...
        "Не стверджуй наявність/ціну для інших країн.\n\n"
        ...
        "Лише якщо явно питають про дзвінки через SIM — розповідай про поповнення/дзвінки.\n\n"
         ...
         # === VIP-номери ===
        "Інколи клієнти можуть запитувати про «гарні», «красиві», «VIP» номери. Відповідай, що в нас є такі номери, але лише по країні Англія. Коштують вони від 1500 грн до 2500 грн. Якщо цікаво, то менеджер надішле вам підбірку.\n\n" 
        ...
        "1. Перевірте чи підключена сім-карта до мережі. Якщо ні, спробуйте підключитись через налаштування в телефоні вручну.\n\n" 
        ...
        "Який оператор для конкретної країни?\nТи не пропонуєш операторів сам. Оператори для країн: Нідерланди - Lebara; Франція - Lebara; Іспанія - Lebara; Чехія - T-Mobile та Kaktus; Польща - Play; Литва - Labas; Казахстан - Tele2; США - Lycamobile.\n\n"
...
def build_manager_parser_prompt() -> str:
    ...
    return (
        ...
        "- ПІБ клієнта.\n"
        "- Номер телефону.\n"
        "- Місто, іноді область, та номер відділення «Нової Пошти», або повну адресу для кур'єрської доставки.\n"
        "- Перелік замовлених SIM-карт (країна та кількість).\n"
        "- Назву оператора (напр. Three, Lebara, O2, Vodafone) для Великобританії.\n"
        "- Сторонні коментарі, які потрібно ігнорувати.\n\n"
        "Правила:\n"
        "1. Твоя відповідь має бути ТІЛЬКИ JSON-об'єктом.\n"
        "2. Для країн використовуй СУВОРО канонічні назви: " + country_keys + ".\n"
        "3. Для Великобританії (Англія) додавай поле `\"operator\"` (O2, Lebara, Vodafone, Three), якщо вказано.\n"
        # --- ПОЧАТОК ЗМІН ---
        "4. **Правило доставки:** Тільки якщо в тексті є явні ключові слова («кур'єр», «адресна»), трактуй доставку як кур'єрську (адресу в `city`, `np` порожній). В іншому випадку, якщо вказана область, клади її в поле `region`. Якщо номер відділення (`np`) не вказано чітко, залишай `city`, `region` та `np` порожніми.\n"
        # --- КІНЕЦЬ ЗМІН ---
        "5. Поля `full_name`, `phone`, `city`, `region`, `np` мають бути рядками. Поле `items` — масивом.\n"
        "6. Якщо дані відсутні, залиш поле як порожній рядок `\"\"` або порожній масив `[]`.\n\n"
        "Приклад:\n"
        "Вхідний текст: 'Іван Франко, тел 0991234567. 2 Англія (водафон), 1 США. Гребінка полтавської обл, відділення 1.'\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == {4}:
        ...
        if forced and forced.items and all([forced.full_name, forced.phone, forced.city]):
