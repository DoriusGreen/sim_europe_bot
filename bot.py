...
ORDER_INFO_REQUEST = (
    ...
    "1. [cite_start]Ім'я та прізвище. [cite: 5]\n"
    "2. [cite_start]Номер телефону. [cite: 5]\n"
    "3. [cite_start]Місто та № відділення \"Нової Пошти\". [cite: 6]\n"
    "4. [cite_start]Країна(и) та кількість sim-карт. [cite: 7]"
)
...
def render_unavailable(unavail: List[str]) -> str:
    ...
    if not names:
        return ""
    if len(names) == 1:
        return f"На жаль, {names[0]} SIM-карти наразі недоступні. [cite_start]У наявності: {available_list_text()}. [cite: 19]"
    return f"На жаль, {', '.join(names)} наразі недоступні. [cite_start]У наявності: {available_list_text()}. [cite: 19, 20]"
...
US_ACTIVATION_MSG = (
    "США, на відміну від інших, потребують поповнення для активації. [cite_start]Після поповнення SIM працюватиме на прийом SMS. [cite: 34, 35]\n\n"
    "Як активувати та поповнити сім-карту США?\n\n"
    "https://www.lycamobile.us/en/activate-sim\n"
    "1. [cite_start]На цьому сайті вводите дані сімки для її попередньої активації. [cite: 36]\n"
    [cite_start]"Отриманий на сайті номер сім-карти записуєте. [cite: 37]\n\n"
    "https://www.lycamobile.us/en/quick-top-up/\n"
    "2. [cite_start]Далі, ось тут, вказавши номер, отриманий на попередньому сайті, поповнюєте сім-карту, після поповнення (мінімум на 23$) вона стане активною та буде приймати SMS. [cite: 38]"
)
...
def build_system_prompt() -> str:
    return (
        # === РОЛЬ ТА КОНТЕКСТ ===
        "Ти — дружелюбний і корисний Telegram-бот-магазин SIM-карт. [cite_start]Чітко та суворо дотримуйся прописаних інструкцій, якщо щось не зрозуміло, то не вигадуй, а краще перепитай клієнта що він мав на увазі. [cite: 53]\n"
        [cite_start]"На початку чату клієнт уже отримує від акаунта власника перелік країн у наявності та інформацію для доставки — ти це НЕ ДУБЛЮЄШ. [cite: 53]\n"
        "Завжди аналізуй кожне повідомлення на наявність елементів замовлення (пункти 1-4 нижче). [cite_start]Якщо в повідомленні або історії є хоча б один пункт, починай процес збору даних: показуй, що бракує, або формуй JSON, якщо все є. [cite: 53, 54]\n"
        [cite_start]"Не чекай явного 'хочу оформити' — починай збір одразу при виявленні даних. [cite: 55]\n\n"
        ...
        "1. [cite_start]Ім'я та прізвище. [cite: 56]\n"
        "2. [cite_start]Номер телефону. [cite: 57]\n"
        "3. [cite_start]Місто та № відділення «Нової Пошти». [cite: 58]\n"
        "4. [cite_start]Країна(и) та кількість sim-карт. [cite: 59]\n\n"

        # === ЯК ПИТАТИ ПРО НЕСТАЧУ ДАНИХ ===
        [cite_start]"Пункт 4 може бути у довільній формі/порядку («Англія 2 шт», «дві UK», «UK x2» тощо). [cite: 59, 60]\n"
        ...
        "2. [cite_start]Номер телефону. [cite: 61]\n"
        "4. [cite_start]Країна(и) та кількість sim-карт. [cite: 62]\n\n"
        ...
        [cite_start]"Для прайсу/наявності доступні ЛИШЕ: ВЕЛИКОБРИТАНІЯ, НІДЕРЛАНДИ, НІМЕЧЧИНА, ФРАНЦІЯ, ІСПАНІЯ, ЧЕХІЯ, ПОЛЬЩА, ЛИТВА, ЛАТВІЯ, КАЗАХСТАН, МАРОККО, США. [cite: 65]\n"
        [cite_start]"Не стверджуй наявність/ціну для інших країн (але довідку USSD можна давати і для інших, якщо відома комбінація). [cite: 66]\n\n"
        ...
        "• Якщо клієнт для Англії називає оператора (O2, Lebara, Vodafone) — додай поле \"operator\" з канонічним значенням; [cite_start]інакше — не додавай це поле. [cite: 67]\n"
        "• Текстові кількості (пара/десяток/кілька) перетворюй у число або попроси уточнення через пункт 4.\n\n"

        # === ЕСКАЛАЦІЯ ДО ЛЮДИНИ ===
        "Запити «зв’язатися з людиною/менеджером/оператором» — це звернення до МЕНЕДЖЕРА магазину. [cite_start]Відповідай: «Очікуйте відповіді менеджера.» [cite: 68]\n"
        "Лише якщо явно питають про дзвінки через SIM — розповідай про поповнення/дзвінки.\n\n"

        # === ПІСЛЯ JSON ===
        "Після JSON бекенд сам рахує суми та формує підсумок. [cite_start]«Загальна сумма» показується лише якщо країн 2+. [cite: 69]\n\n"
        ...
        "Так! [cite_start]Ви одразу зможете зареєструвати WhatsApp, Telegram, Viber та інші, а також прийняти SMS. [cite: 70]\n\n"
        "Чи потрібно поповнювати?\n"
        "Ні. [cite_start]Сім-карта працює на прийом SMS, але для дзвінків потрібне поповнення (ding.com, PayPal). [cite: 71]\n\n"
        "Скільки SIM-карта буде активна?\n"
        "Зазвичай до півроку; [cite_start]щоб довше — раз на 6 міс поповнюйте на 10 фунтів/євро. [cite: 72]\n\n"
        "Які тарифи?\n"
        ...
        "Чи даєте гарантії?\n"
        [cite_start]"Якщо SIM не працюватиме (рідко), зробимо заміну або повернемо кошти. [cite: 73]\n\n"
        "Коли відправите?\n"
        "Зазвичай у день замовлення або протягом 24 годин.\n\n"
        "Чи вже відправили? Чи є ТТН/трек-номер?\n"
        "Зазвичай відправляємо в день замовлення або протягом 24 годин. [cite_start]Якщо потрібна ТТН — очікуйте відповіді менеджера. [cite: 74, 75]\n\n"
        "Як оплатити?\n"
        "Зазвичай накладений платіж. [cite_start]За бажанням — карта або USDT (TRC-20). [cite: 76]\n\n"
        "Чи можлива відправка в інші країни?\n"
        "Так, від 3 шт, повна передоплата, «Нова Пошта».\n\n"
        "Який оператор для конкретної країни?\n"
        "Ти не пропонуєш операторів сам, тільки відповідаєш, коли клієнт сам конкретно запитує про оператора для країни. [cite_start]Якщо клієнт запитує про неіснуючого оператора для країни (наприклад, Vodafone для Німеччини, але є тільки Lebara), скажи, що в роботі оператор Lebara (або той, що є) нічим не відрізняється. [cite: 77] Оператори для країн: Нідерланди - Lebara; Франція - Lebara; Іспанія - Lebara; [cite_start]Чехія - T-Mobile та Kaktus; [cite: 78] Польща - Play (але потрібно уточнити у менеджера); Литва - Labas; [cite_start]Казахстан - Tele2; [cite: 79] [cite_start]США - Lycamobile. [cite: 80]\n\n"

        # === США — ОСОБЛИВО ===
        "США — на відміну від інших, потребують поповнення для активації. [cite_start]Після поповнення SIM працюватиме на прийом SMS. [cite: 81]\n\n"
...
def build_followup_prompt() -> str:
    return (
        ...
        [cite_start]"Якщо користувач просить «зв’язатися з людиною/менеджером/оператором» — це звернення до МЕНЕДЖЕРА. Відповідай: «Очікуйте відповіді менеджера.» [cite: 82]\n\n"
...
def build_force_point4_prompt() -> str:
    return (
        [cite_start]"У контексті вже є пункти 1–3 (ПІБ, телефон, місто+№). [cite: 82]\n"
        [cite_start]"Останнє повідомлення, включно з процитованим, ймовірно містить лише пункт 4 (країни та кількість). [cite: 83]\n"
        [cite_start]"Витягни пункт 4, поєднай з 1–3 з контексту і ПОВЕРНИ ЛИШЕ ПОВНИЙ JSON замовлення. [cite: 84]"
    )
...
def _soft_phone_spaces(phone: str) -> str:
    """
    М'яко приводить телефон до пробілів: '098-666-777' -> '098 666 777'. [cite_start]Якщо format_phone вже зробив 3-3-4 — залишає як є. [cite: 88, 89]
    """
...
def _parse_city_and_np(line: str) -> Tuple[str, str]:
    """
    Витягує місто та № НП з рядка. [cite_start]Підтримує 'Київ № 377', 'Львів 377', 'Одеса No 12', 'Dnipro #45' [cite: 89, 90]
    """
...
def render_order_for_group(order: OrderData, paid: bool) -> str:
    """
    [cite_start]Спеціальний рендер для групи: без «дякуємо» та, якщо paid=True, замість ціни пише '(замовлення оплачене)'. [cite: 94, 95]
    """
    lines = []
    grand_total = 0
    counted = 0
...
async def _openai_chat(messages: List[Dict[str, str]]) -> str:
    try:
        ...
        return "Вибачте, сталася технічна помилка. [cite_start]Спробуйте, будь ласка, ще раз. [cite: 105]"
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        "Вітаю! [cite_start]Я допоможу вам оформити замовлення на SIM-карти, а також постараюсь надати відповіді на всі ваші запитання. [cite: 109]"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # [cite_start]Знімаємо ігнор тільки для цієї групи: парсимо запис, видаляємо оригінал, публікуємо красиво. [cite: 109, 110]
    if (
        ...
        if parsed:
            ...
            # [cite_start]Спробуємо видалити оригінальне повідомлення менеджера (потрібні права адміна) [cite: 111]
            try:
    ...
    if reply_text.strip().startswith("🛒 Для оформлення замовлення") and context.chat_data.get("awaiting_missing") == {1, 2, 3}:
        reply_text = (
            "📝 Залишилось вказати:\n\n"
            "1. [cite_start]Ім'я та прізвище. [cite: 128]\n"
            "2. [cite_start]Номер телефону. [cite: 129]\n"
            "3. [cite_start]Місто та № відділення \"Нової Пошти\". [cite: 130]\n"
        )
