...
def render_price_block(country_key: str) -> str:
   ...
   tiers = PRICE_TIERS.get(country_key, [])
    ...
    for idx, (min_q, price) in enumerate(tiers_sorted):
        ...
        if not 
inserted_gap and min_q >= 100 and idx > 0:
            lines.append("")
            inserted_gap = True
...
US_ACTIVATION_MSG = (
    "США, на відміну 
від інших країн, потребують поповнення для активації. Після поповнення SIM працюватиме на прийом SMS.\n\n"
    "Як активувати та поповнити сім-карту США?\n\n"
    "https://www.lycamobile.us/en/activate-sim\n"
    "1.
На цьому сайті вводите дані сімки для її попередньої активації.\n"
    "Отриманий на сайті номер сім-карти записуєте.\n\n"
    "https://www.lycamobile.us/en/quick-top-up/\n"
    "2.
Далі, ось тут, вказавши номер, отриманий на попередньому сайті, поповнюєте сім-карту, після поповнення (мінімум на 23$) вона стане активною та буде приймати SMS."
)
...
def render_ussd_targets(targets: List[Dict[str, str]]) -> str:
    ...
    for t in targets:
        ...
        if op_req and pairs:
            pairs = [p for p in pairs if (p[0] and canonical_operator_any(p[0]) == 
op_req)]
        if not pairs:
            if op_req:
...
def _country_mentions_with_pos(text: str) -> List[Tuple[str, int]]:
    ...
    for key, 
subs in COUNTRY_KEYWORDS.items():
        best = None
        for s in subs:
            i = low.find(s)
            if i != -1:
...
Якщо в повідомленні або історії є хоча б один пункт, починай процес збору даних: показуй, що бракує, або формуй JSON, якщо все є.\n"
       ...
       "ПОВНЕ замовлення складається з 4 пунктів:\n"
        "1.
Ім'я та прізвище (Не плутай ім'я клієнта з по-батькові! Записуй лише імя та прізвище.).\n"
        "2.
Номер телефону.\n"
        "3.
Адреса доставки. Це може бути один з двох варіантів:\n"
        "   - Місто та № відділення «Нової Пошти».\n"
...
"city": "Місто або Повна Адреса",\n'
        ...
        "{\n"
        '  "ask_prices": true,\n'
        '  "countries": ["ALL" або перелік 
ключів, напр. "ВЕЛИКОБРИТАНІЯ","США"]\n'
        "}\n\n"
...
Ось яку інформацію в цьому випадку ти маєш їм відправити:\n\n" 
        "1.
Перевірте чи підключена сім-карта до мережі. Якщо ні, спробуйте підключитись через налаштування в телефоні вручну (просто виберіть будь-яку мережу із запропонованих).\n\n" 
        "2.
Якщо мережа є, але СМС не приходить, то це точно проблема на стороні сервісу, і потрібно спробувати пізніше, бажано через 24 години.\n\n" 
        "3.
Щоб переконатись в тому, що сім-карта точно приймає СМС, можете спробувати відправити повідомлення з українського номеру.\n\n" 
...
def is_meaningful_followup(text: str) -> bool:
    ...
    if not t:
        return False
    low = t.lower()
    banned_words = ["ціни", "прайс", "надіслано", "див.
вище", "вище", "повторю"]
...
Можуть вказуватись кирилицею, чи скорочено.\n"
        ...
        "Правила:\n"
        "1.
Твоя відповідь має бути ТІЛЬКИ JSON-об'єктом. Без жодних пояснень, тексту до чи після, чи markdown-форматування.\n"
        "2.
Для країн використовуй СУВОРО канонічні назви з цього списку: " + country_keys + ".\n"
        "3.
Якщо для країни ВЕЛИКОБРИТАНІЯ (Англія) вказаний оператор (напр. O2, Lebara, Vodafone, Three), додай в об'єкт товару поле `\"operator\"`.
Використовуй канонічне значення: `\"O2\"`, `\"Lebara\"`, `\"Vodafone\"`, `\"Three\"`. Якщо оператор не вказаний, не додавай це поле.\n"
        "4.
Поля `full_name`, `phone`, `city`, `np` мають бути рядками. Поле `items` — масивом об'єктів.\n"
        "5.
Якщо якісь дані відсутні в тексті, залиш відповідне поле як порожній рядок \"\" або порожній масив [].\n\n"
        "Приклад:\n"
        "Вхідний текст: 'Так, це новий клієнт, Іван Франко, тел 0991234567. Хоче 2 сімки для Англії оператора водафон та 1 для США.
Відправка в Київ, відділення 30. Каже, що оплатить на карту.'\n"
...
async def _ask_gpt_to_parse_manager_order(text: str) -> str:
 ...
 response = await openai.ChatCompletion.acreate(
        ...
        logger.error(f"Помилка GPT-парсера для 
менеджера: {e}")
        return ""
...
async def _ask_gpt_force_point4(history: List[Dict[str, str]], user_payload: str) -> str:
    messages = 
[{"role": "system", "content": build_force_point4_prompt()}]
    messages.extend(history)
    messages.append({"role": "user", "content": user_payload})
    try:
        response = await openai.ChatCompletion.acreate(
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.reply_text(
        "Вітаю!
Я допоможу вам оформити замовлення на SIM-карти, а також постараюсь надати відповіді на всі ваші запитання."
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Видалено фільтр is_ack_only. Всі повідомлення 
йдуть на обробку.
...
parts[0]
    ...
    if quoted_text:
        user_payload = (
            raw_user_message
            + "\n\n[ЦЕ ПРОЦИТОВАНЕ ПОВІДОМЛЕННЯ КЛІЄНТА (вважай частиною актуальних 
даних):]\n"
            + quoted_text
        )
...
"
    ...
    if p4_items:
        pairs_text = ";
".join(f"{DISPLAY.get(c, c.title())} — {q}" for c, q in p4_items)
        user_payload += (
...
if context.chat_data.get("awaiting_missing") == {1, 2, 3} and context.chat_data.get("point4_hint"):
     ...
     f"Додай/склей із пунктами 1–3.]"
            ...
            pairs_text = ";
".join(f"{DISPLAY.get(c, c.title())} — {q}" for c, q in h["items"])
            user_payload += (
    ...
    if awaiting == {4}:
        force_json = 
await _ask_gpt_force_point4(history, user_payload)
...
😊")
    ...
    if "Залишилось вказати:" in reply_text and "📝 Залишилось вказати:" not 
in reply_text:
...
😊")
    ...
    if price_countries is not None:
      ...
      history.append({"role": "user", "content": raw_user_message})
            ...
            unavailable_msg 
= render_unavailable(invalid if invalid else price_countries)
            history.append({"role": "user", "content": raw_user_message})
...
msg.reply_text(reply_text)
        ...
        logger.info(f"GPT повернув пусту відповідь на повідомлення: '{raw_user_message}'.
