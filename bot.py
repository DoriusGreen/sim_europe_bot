...
def build_system_prompt() -> str:
    return (
        ...
        # --- ПОЧАТОК ЗМІН ---
        "=== ПРАВИЛО КУР'ЄРСЬКОЇ ДОСТАВКИ ===\n"
        "Ти повинен розпізнавати кур'єрську (адресну) доставку ТІЛЬКИ ЯКЩО клієнт прямо про це написав, використовуючи ключові слова, наприклад: «кур'єром», «адресна доставка», «доставка на адресу», «додому».\n"
        "ТІЛЬКИ за наявності цих слів ти записуєш повну адресу в поле `city`, а поле `np` залишаєш порожнім.\n"
        "Якщо клієнт просто вказав адресу без цих ключових слів, це адреса ВІДДІЛЕННЯ. Спробуй розібрати місто та номер. Якщо номер незрозумілий — не вигадуй, а краще залиш поля доставки порожніми, щоб спрацювала логіка запиту уточнення.\n\n"
        # --- КІНЕЦЬ ЗМІН ---
...
def build_manager_parser_prompt() -> str:
    ...
    return (
        ...
        # --- ПОЧАТОК ЗМІН ---
        "4. Тільки якщо в тексті є явні ключові слова (напр. «кур'єр», «адресна»), трактуй доставку як кур'єрську. В такому випадку записуй всю адресу в поле `\"city\"`, а поле `\"np\"` залишай порожнім. В іншому випадку — очікуй місто та номер відділення.\n"
        # --- КІНЕЦЬ ЗМІН ---
