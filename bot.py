...
def build_system_prompt() -> str:
    ...
    return (
        ...
        # === ОСОБЛИВІ ВИПАДКИ ===
        "ВАЖЛИВО: Оператори Lebara, O2 та Three (3) для Англії (Великобританії) більше не доступні. Якщо клієнт запитує саме про них, чи питає які є оператори для Англії, не додавай їх до замовлення і не генеруй JSON. Замість цього, відповідай СУВОРО таким текстом:\n"
        "Наразі в наявності англійський оператор Vodafone, всі інші (Lebara, O2, Three) перестали працювати в Україні.\n\n"
        "Як альтернативу пропонуємо англійський оператор Vodafone (+44). Наразі це один з найстабільніших в роботі операторів.\n\n"
        "*#100# - дізнатись номер англійського водафону.\n\n"
        ...
        # === США — НЕ ДОСТУПНІ ===
        "ВАЖЛИВО: SIM-карти США більше не доступні. "
        "Якщо клієнт запитує про наявність, ціну, активацію або хоче замовити SIM-карти США (Америка, Штати, USA тощо) — НЕ додавай їх до замовлення і НЕ генеруй JSON.\n"
        "Замість цього, відповідай СУВОРО таким текстом:\n"
        "На жаль, США перестали працювати в Україні, тому зараз їх не пропонуємо. Радимо звернути увагу на сім-карти Європи.\n"
    )
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ussd_targets is not None:
        if ussd_targets:
            ...
            clarification_text = "Будь ласка, уточніть, для якої країни вам потрібна USSD-комбінація?"
            history.append({"role": "user", "content": raw_user_message})
            history.append({"role": "assistant", "content": clarification_text})
            _prune_history(history)
            await msg.reply_text(clarification_text)
            return
    # --- КІНЕЦЬ ЗМІН ---
