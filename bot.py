...
def build_system_prompt() -> str:
    return (
        ...
        "Зазвичай відправляємо в день замовлення або протягом 24 годин. Якщо потрібен ТТН/трек-номер — очікуйте відповіді оператора (менеджера).\n\n"
        "Як оплатити?\n"
...
def build_followup_prompt() -> str:
    return (
        ...
        "«Зазвичай відправляємо в день замовлення або протягом 24 годин. Якщо потрібен ТТН/трек-номер — очікуйте відповіді оператора.»"
    )
...
def main():
    if not TELEGRAM_TOKEN or not OPENAI_API_KEY or not WEBHOOK_URL:
        raise RuntimeError("Не задано TELEGRAM_BОT_TOKEN, OPENAI_API_KEY або WEBHOOK_URL")
