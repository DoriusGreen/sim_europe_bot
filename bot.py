...
def build_system_prompt() -> str:
    ...
    return (
        ...
        "суму за країною (qty × unit). У «Загальна сумма» — підсумовуй усі країни, але тільки якщо їх 2+.\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # --- Патч 1: якщо модель віддала старий заголовок без емодзі — виправляємо ---
    ...
    # --- Патч 2: приховати «Загальна сумма», якщо країн лише одна ---
    lines = reply_text.splitlines()
    flag_lines = [ln for ln in lines if any(ln.strip().startswith(f) for f in FLAGS)]
    if len(flag_lines) <= 1:
        lines = [ln for ln in lines if not ln.strip().lower().startswith("загальна сумма:")]
        reply_text = "\n".join(lines)
