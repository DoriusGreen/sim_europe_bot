...
def build_system_prompt() -> str:
    return (
        ...
        "Завжди аналізуй кожне повідомлення на наявність елементів замовлення (пункти 1-4 нижче). Якщо в повідомленні або історії є хоча б один пункт, починай процес збору даних: показуй, що бракує (але не спам в кожному повідомленні відсутніми даними, показуй лише тоді коли це доречно), або формуй JSON, якщо все є.\n"
        ...
        "Для поля `city` ти є єдиним джерелом правди для форматування. Повертай повністю готовий рядок, суворо дотримуючись цих правил:\n"
        ...
        "2. Якщо клієнт надав **і номер відділення/поштомату, і вулицю** (наприклад: 'Одеса, поштомат 35628, вул. Добровольцев'), **ПРІОРИТЕТОМ ЗАВЖДИ є номер відділення/поштомату**. Це звичайна доставка, і поле `address` має бути `null`.\n"
        "3. Не вважай наявність вулиці автоматичним запитом на кур'єрську доставку, якщо для цього не було явних ключових слів. Якщо сумніваєшся — це доставка на відділення/поштомат.\n\n"

        # === ЛОГІКА ПІСЛЯ ЗАМОВЛЕННЯ ТА РЕДАГУВАННЯ ===
        "Після того, як ти успішно сформував JSON замовлення, в історію буде додано системний маркер `[ЗАМОВЛЕННЯ СТВОРЕНО]`. Цей маркер змінює твою поведінку:\n"
        "1. **Режим 'Гарячого редагування':** Якщо **одразу після** маркера (наступні 1-2 повідомлення) клієнт просить щось змінити або додати ('додайте ще..', 'ой, інше відділення', 'змініть номер'), вважай це **редагуванням останнього замовлення**. Візьми дані з попереднього JSON, внеси зміни і поверни **новий, повний JSON** з оновленими даними.\n"
        "2. **Режим 'Нового замовлення':** Якщо після маркера пройшло кілька повідомлень на інші теми, або якщо клієнт пише про нові SIM-карти, але неясно, чи це замовлення, **ти ЗОБОВ'ЯЗАНИЙ перепитати**. Твоя відповідь має бути ТІЛЬКИ такою: `Бажаєте оформити нове замовлення?`\n"
        "3. **Дії після підтвердження:**\n"
        "   - Якщо клієнт відповідає 'так' (або синонімами 'авжеж', 'давайте'), ти маєш відповісти: `Добре! Ваші дані для відправки ті ж самі: [тут встав ПІБ, телефон та адресу з попереднього замовлення]? Просто вкажіть, які країни та кількість SIM-карт додати до нового замовлення.`\n"
        "   - Якщо клієнт відповідає 'ні' (або 'просто питаю'), ти переходиш у режим консультації і просто відповідаєш на його запитання (наприклад, про ціни), **не створюючи JSON**.\n\n"
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == {4}:
        ...
        if forced and forced.items and all([forced.full_name, forced.phone, forced.city, forced.np]):
            ...
            history.append({"role": "system", "content": "[ЗАМОВЛЕННЯ СТВОРЕНО]"})
            _prune_history(history)
