...
def _is_manager_message(msg: Message) -> bool:
    ...
    if MANAGER_USER_IDS and u.id in MANAGER_USER_IDS: return True
    if MANAGER_USERNAMES and u.username and u.username.lower() in MANAGER_USERNAMES: return True
    return False

# ===== Відкладене надсилання у групу (ASYNCIO, без JobQueue) =====
FORWARD_DELAY_SEC = 60  # <- 60 сек

async def _forward_task(context: ContextTypes.DEFAULT_TYPE, chat_id: int, summary: str, task_id: int):
    try:
        await asyncio.sleep(FORWARD_DELAY_SEC)
        # Надсилаємо тільки якщо це ще актуальна задача
        if context.chat_data.get("pending_forward_task_id") != task_id:
            logger.info("forward_task: скасовано (task_id змінився)")
            return
        logger.info("forward_task: надсилаю замовлення у групу %s", ORDER_FORWARD_CHAT_ID)
        await context.bot.send_message(chat_id=ORDER_FORWARD_CHAT_ID, text=summary)
    except asyncio.CancelledError:
        logger.info("forward_task: task cancelled")
    except Exception as e:
        logger.warning(f"Не вдалося надіслати замовлення в групу: {e}")
    finally:
        context.chat_data.pop("pending_forward_task", None)
        context.chat_data.pop("pending_forward_task_id", None)
        context.chat_data.pop("pending_forward_summary", None)
        context.chat_data.pop("pending_forward_created_at", None)

def _cancel_pending_forward(context: ContextTypes.DEFAULT_TYPE) -> None:
    task: Optional[asyncio.Task] = context.chat_data.pop("pending_forward_task", None)
    if task and not task.done():
        task.cancel()

def _schedule_forward(context: ContextTypes.DEFAULT_TYPE, chat_id: int, summary: str) -> None:
    """Переплановуємо тільки при новому підсумку."""
    _cancel_pending_forward(context)
    task_id = int(time.time() * 1000)
    context.chat_data["pending_forward_task_id"] = task_id
    context.chat_data["pending_forward_summary"] = summary
    context.chat_data["pending_forward_created_at"] = time.time()
    task = context.application.create_task(_forward_task(context, chat_id, summary, task_id))
    context.chat_data["pending_forward_task"] = task
    logger.info("forward_task: заплановано через %s сек", FORWARD_DELAY_SEC)

# ==== Повідомлення/прайси/прапорці та ін. ====
ORDER_INFO_REQUEST = (
...
FLAGS = {
    "ВЕЛИКОБРИТАНІЯ": "🇬🇧", "НІДЕРЛАНДИ": "🇳🇱", "НІМЕЧЧИНА": "🇩🇪", "ФРАНЦІЯ": "🇫🇷",
    "ІСПАНІЯ": "🇪🇸", "ЧЕХІЯ": "🇨🇿", "ПОЛЬЩА": "🇵🇱", "ЛИТВА": "🇱🇹", "ЛАТВІЯ": "🇱🇻",
    "КАЗАХСТАН": "🇰🇿", "МАРОККО": "🇲🇦", "США": "🇺🇸", "ІТАЛІЯ": "🇮🇹", "МОЛДОВА": "🇲🇩",
}
DISPLAY = {
    "ВЕЛИКОБРИТАНІЯ": "Англія", "НІДЕРЛАНДИ": "Нідерланди", "НІМЕЧЧИНА": "Німеччина",
    "ФРАНЦІЯ": "Франція", "ІСПАНІЯ": "Іспанія", "ЧЕХІЯ": "Чехія", "ПОЛЬЩА": "Польща",
    "ЛИТВА": "Литва", "ЛАТВІЯ": "Латвія", "КАЗАХСТАН": "Казахстан", "МАРОККО": "Марокко",
    "США": "США", "ІТАЛІЯ": "Італія", "МОЛДОВА": "Молдова",
}
DIAL_CODES = {
    "ВЕЛИКОБРИТАНІЯ": "+44", "ІСПАНІЯ": "+34", "ФРАНЦІЯ": "+33", "НІМЕЧЧИНА": "+49",
    "НІДЕРЛАНДИ": "+31", "ІТАЛІЯ": "+39", "ЧЕХІЯ": "+420", "МОЛДОВА": "+373",
    "КАЗАХСТАН": "+7", "ЛАТВІЯ": "+371", "США": "+1",
}
...
def render_price_block(country_key: str) -> str:
    ...
    for idx, (min_q, price) in enumerate(tiers_sorted):
        ...
        if not inserted_gap and min_q >= 100 and idx > 0:
            lines.append(""); inserted_gap = True
...
def render_prices(countries: List[str]) -> str:
    return "".join(render_price_block(normalize_country(c).upper()) for c in countries if normalize_country(c).upper() in PRICE_TIERS)
...
def render_unavailable(unavail: List[str]) -> str:
    ...
    return (f"На жаль, {names[0]} SIM-карти наразі недоступні. У наявності: {available_list_text()}."
            if len(names) == 1 else
            f"На жаль, {', '.join(names)} наразі недоступні. У наявності: {available_list_text()}.")

def _cap_word(w: str) -> str: return w[:1].upper() + w[1:].lower() if w else w
...
def format_full_name(name: str) -> str:
    ...
    return f"{_smart_title(tokens[0])} {_smart_title(tokens[-1])}" if len(tokens) >= 2 else _smart_title(tokens[0])
...
def _split_city_extra(city: str):
    ...
    return (parts[0], parts[1]) if len(parts) == 2 else (s, None)

def format_city(city: str) -> str:
    base, extra = _split_city_extra(city)
    base_fmt = _smart_title(base)
    return f"{base_fmt} ({_smart_title(extra)})" if extra else base_fmt
    ...
    try:
        ...
        if data.get("ask_ussd") is True and isinstance(data.get("targets"), list):
            ...
            for t in data["targets"]:
                ...
                c = t.get("country"); o = t.get("operator")
                if c: out.append({"country": c, "operator": o})
            return out or None
        return None
    except Exception: return None
...
def user_mentions_usa(text: str) -> bool:
    t = (text or "").lower()
    return bool(re.search(r"\b(сша|usa|u\.s\.a\.|united states|штат[а-яіїє]+|америк[аи])\b", t) or re.search(r"(^|\s)\+1(\s|$)", t))
...
def render_ussd_targets(targets: List[Dict[str, str]]) -> str:
    ...
    for t in targets:
        ...
        if not pairs:
            if op_req: result_lines.append(f"{code_prefix} {flag} {disp} (оператор {op_req}) — {FALLBACK_PLASTIC_MSG}")
            else:      result_lines.append(f"{code_prefix} {flag} {disp} — {FALLBACK_PLASTIC_MSG}")
            result_lines.append(""); continue
        for op, code in pairs:
            if op: result_lines.append(f"{code_prefix} {flag} {disp} (оператор {op}) — {code}")
            else: result_lines.append(f"{code_prefix} {flag} {disp} — {code}")
        result_lines.append("")
...
def build_system_prompt() -> str:
    return (
        ...
        "Завжди аналізуй кожне повідомлення на наявність елементів замовлення (пункти 1-4 нижче). Якщо в повідомленні або історії є хоча б один пункт, починай процес збору даних.\n\n"
        ...
        "Якщо виявлено хоча б один пункт, але ще НЕ вказано ВСІ 4 — відповідай СУВОРО так:\n"
        "📝 Залишилось вказати:\n\n<залиши лише відсутні рядки з їхніми номерами>\n\n"
        "Коли ВСІ дані є — ВІДПОВІДАЙ ЛИШЕ JSON:\n{\n"
        '  "full_name":"…", "phone":"…", "city":"…", "np":"…",\n'
        '  "items":[{"country":"КРАЇНА","qty":N,"operator":"O2|Lebara|Vodafone"},...]\n}\n\n'
        "Якщо просять ПРО ЦІНИ/наявність — ВІДПОВІДАЙ ЛИШЕ JSON:\n{\n"
        '  "ask_prices": true,\n  "countries": ["ALL" або перелік ключів]\n}\n\n'
        "Як дізнатись номер — ВІДПОВІДАЙ ЛИШЕ JSON:\n{\n"
        '  "ask_ussd": true,\n  "targets":[{"country":"КРАЇНА","operator":"…(необов’язково)"}]\n}\n\n'
        "Доступні для продажу: ВЕЛИКОБРИТАНІЯ, НІДЕРЛАНДИ, НІМЕЧЧИНА, ФРАНЦІЯ, ІСПАНІЯ, ЧЕХІЯ, ПОЛЬЩА, ЛИТВА, ЛАТВІЯ, КАЗАХСТАН, МАРОККО, США.\n\n"
        ...
        "США — потребують поповнення для активації. Якщо питають про США — надсилай інструкцію (її текст відомий бекенду).\n"
    )

def build_followup_prompt() -> str:
    return (
        "Прайс/відповідь уже надіслано окремо. Відповідай коротко на інші частини.\n"
        "Якщо просили лише ціну — поверни порожній рядок. Не пиши «підтверджую наявність» тощо.\n"
        "Ніколи не виводь довільний JSON/код, окрім JSON для USSD.\n"
        "Шаблон USSD JSON:\n{\n  \"ask_ussd\": true,\n  \"targets\": [{\"country\":\"КРАЇНА\",\"operator\":\"…\"}]\n}\n"
    )

def build_force_point4_prompt() -> str:
    return ("У контексті вже є пункти 1–3. Останнє повідомлення містить пункт 4. "
            "Склей і поверни лише ПОВНИЙ JSON замовлення.")

ACK_PATTERNS = [
    r"^\s*(ок(ей)?|добре|чудово|гарно|дякую!?|спасибі|спасибо|жду|чекаю)\s*[\.\!]*\s*$",
    r"^\s*[👍🙏✅👌]+\s*$",
]
def is_ack_only(text: str) -> bool:
    if not text: return False
    low = text.strip().lower()
    return any(re.match(p, low) for p in ACK_PATTERNS)
...
def _looks_like_code_or_json(s: str) -> bool:
    t = (s or "").strip()
    return bool(t and (t.startswith("```") or t.lower().startswith("```json") or t.startswith("{") or t.lower().startswith("json")))
...
async def _openai_chat(messages: List[Dict[str, str]]) -> str:
    try:
        r = openai.ChatCompletion.create(model="gpt-4o", messages=messages, max_tokens=600, temperature=0.2)
...
async def _ask_gpt_main(history: List[Dict[str, str]], user_payload: str) -> str:
    return await _openai_chat([{"role":"system","content":build_system_prompt()}, *history, {"role":"user","content":user_payload}])

async def _ask_gpt_followup(history: List[Dict[str, str]], user_payload: str) -> str:
    messages = [{"role":"system","content":build_followup_prompt()}, *(history[-4:] if len(history)>4 else history), {"role":"user","content":user_payload}]
    try:
        r = openai.ChatCompletion.create(model="gpt-4o", messages=messages, max_tokens=300, temperature=0.2)
...
async def _ask_gpt_force_point4(history: List[Dict[str, str]], user_payload: str) -> str:
    messages = [{"role":"system","content":build_force_point4_prompt()}, *history, {"role":"user","content":user_payload}]
    try:
        r = openai.ChatCompletion.create(model="gpt-4o", messages=messages, max_tokens=500, temperature=0.1)
        return r.choices[0].message["content"].strip()
    except Exception as e:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = update.effective_message
    if not msg:
        logger.warning("No effective_message in update: %s", update); return

    raw_user_message = msg.text.strip() if msg.text else ""
    history = _ensure_history(context)

    # Менеджер: не відповідаємо, але пишемо в історію
    ...
    if qty_only and last_price_countries:
        ...
        user_payload += f"\n\n[ПІДСКАЗКА: кількість {qty_only} шт, застосуй до: {hint_countries}. Це пункт 4.]"
    ...
    # країни+кількості в одному повідомленні
    p4_items = detect_point4_items(raw_user_message)
    if p4_items:
        pairs_text = "; ".join(f"{DISPLAY.get(c,c.title())} — {q}" for c,q in p4_items)
        user_payload += f"\n\n[ПІДСКАЗКА: пункт 4 знайдено тут: {pairs_text}. Склей з 1–3 та поверни ПОВНИЙ JSON.]"
        context.chat_data["awaiting_missing"] = {1,2,3}
    ...
    if context.chat_data.get("awaiting_missing") == {1,2,3} and context.chat_data.get("point4_hint"):
        ...
        if "qty" in h:
            user_payload += f"\n\n[НАГАДУВАННЯ: 4 вже відомий — {', '.join(h['countries'])} по {h['qty']} шт.]"
        elif "items" in h:
            pairs_text = "; ".join(f"{DISPLAY.get(c,c.title())} — {q}" for c,q in h["items"])
            user_payload += f"\n\n[НАГАДУВАННЯ: 4 вже відомий — {pairs_text}]"
    ...
    if price_countries is not None:
        ...
        context.chat_data["last_price_countries"] = list(PRICE_TIERS.keys()) if want_all else (valid[:] if valid else [])
        if want_all:
            price_msg = "".join(render_price_block(k) for k in PRICE_TIERS.keys())
            history.append({"role":"user","content":raw_user_message}); history.append({"role":"assistant","content":price_msg}); _prune_history(history)
            await msg.reply_text(price_msg)
        elif valid:
            price_msg = render_prices(valid)
            history.append({"role":"user","content":raw_user_message}); history.append({"role":"assistant","content":price_msg}); _prune_history(history)
            await msg.reply_text(price_msg)
            if invalid: await msg.reply_text(render_unavailable(invalid))
        else:
            unavailable_msg = render_unavailable(invalid if invalid else price_countries)
            history.append({"role":"user","content":raw_user_message}); history.append({"role":"assistant","content":unavailable_msg}); _prune_history(history)
            await msg.reply_text(unavailable_msg)

        usa_intent = (("США" in valid) and (len(valid) == 1 or user_mentions_usa(raw_user_message)))
        if usa_intent: await msg.reply_text(US_ACTIVATION_MSG)
        ...
        if ussd_targets:
            ...
            history.append({"role":"assistant","content":formatted}); _prune_history(history)
            context.chat_data.pop("awaiting_missing", None); context.chat_data.pop("point4_hint", None)
            await msg.reply_text(formatted)
            return
        if _looks_like_code_or_json(follow): follow = ""
        if is_meaningful_followup(follow):
            if not contains_us_activation_block(follow):
                history.append({"role":"assistant","content":follow}); _prune_history(history)
    ...
    if ussd_targets:
        formatted = render_ussd_targets(ussd_targets) or FALLBACK_PLASTIC_MSG
        history.append({"role":"user","content":raw_user_message}); history.append({"role":"assistant","content":formatted}); _prune_history(history)
        context.chat_data.pop("awaiting_missing", None); context.chat_data.pop("point4_hint", None)
        await msg.reply_text(formatted)
        return
