...
def build_followup_prompt() -> str:
    return (
        ...
        "Не повторюй і не перефразовуй ціни, не надсилай JSON. "
        "Не пиши фрази на кшталт «ціни надіслано/див. вище» — якщо інших питань немає, "
        "ПОВЕРНИ ПУСТИЙ РЯДОК (без жодного символу). "
        "Якщо питали про терміни відправки — відповідай: «Відправляємо протягом 24 годин.» "
        "Якщо питали про наявність — коротко підтверди наявність. "
        "Одна-дві фрази максимум."
    )

# ---- допоміжний фільтр, щоб не відсилати «порожні» або службові відповіді
def is_meaningful_followup(text: str) -> bool:
    ...
    low = t.lower()
    banned = [
        "ціни", "прайс", "прайсу", "надіслано", "надіслав", "відправив",
        "дивіться вище", "дивись вище", "див. вище", "вище", "повторю"
    ]
    if any(w in low for w in banned):
        return False
    # якщо випадково знову прилетів прайс (суми/шт/грн) — не шлемо
    if "грн" in low or re.search(r"\bшт\.?\b", low):
        return False
    # дуже короткі слова типу "так"/"ні" теж відсікаємо
    if len(t) < 4:
        return False
    return True
# --------------------------------------------------------------------
