...
def missing_points_from_reply(text: str) -> Set[int]:
    """Повертає набір пунктів (1..4), які модель просить ще вказати."""
    out: Set[int] = set()
...
def build_followup_prompt() -> str:
    return (
        ...
        "Якщо питали про терміни відправки — «Відправляємо протягом 24 годин.»; якщо про наявність — підтверди."
    )
...
def build_force_point4_prompt() -> str:
    return (
        ...
        "наприклад: «Англія 2 шт», «2 штуки англія», «дві UK», «UK x2» тощо. "
        "Твоє завдання — витягти пункт 4, поєднати з пунктами 1–3 з контексту і ПОВЕРНУТИ ЛИШЕ ПОВНИЙ JSON замовлення за схемою:\n"
        "{\n"
        '  \"full_name\": \"...\",\n'
        '  \"phone\": \"...\",\n'
        '  \"city\": \"...\",\n'
        ...
        '  \"items\": [ {\"country\":\"КРАЇНА\",\"qty\":N,\"operator\":\"O2|Lebara|Vodafone\"?}, ... ]\n'
        "}\n"
        "Країни повертай ключами: ВЕЛИКОБРИТАНІЯ, НІДЕРЛАНДИ, НІМЕЧЧИНА, ФРАНЦІЯ, ІСПАНІЯ, ЧЕХІЯ, ПОЛЬЩА, ЛИТВА, ЛАТВІЯ, КАЗАХСТАН, МАРОККО, США. "
        "Якщо для Англії вказано оператора — додай operator (O2/Lebara/Vodafone). Без зайвого тексту."
    )

# ---- допоміжний фільтр для фоллоу-ап
...
async def _ask_gpt_force_point4(history: List[Dict[str, str]], user_message: str) -> str:
    ...
    messages.extend(history)  # важливо: тут потрібен увесь контекст, щоб дістати 1–3
    messages.append({"role": "user", "content": user_message})
    try:
        response = openai.ChatCompletion.create(
            model="gpt-4o",
