...
def _order_signature(order: OrderData) -> str:
    ...
    # Використовуємо необроблені дані, щоб уникнути проблем з форматуванням повної адреси
...
def render_order(order: OrderData) -> str:
    ...
    # --- ПОЧАТОК ЗМІН ---
    delivery_line = ""
    # Якщо є номер відділення - це стандартна доставка
    if order.city and order.np:
        delivery_line = f"{format_city(order.city)} № {format_np(order.np)}"
    # Якщо є тільки 'city' (де може бути повна адреса) - це кур'єрська
    elif order.city:
        delivery_line = f"{order.city.strip()}" # Виводимо як є, без змін
...
def build_system_prompt() -> str:
    return (
        ...
        "1. **Стандартна доставка:** За замовчуванням ти очікуєш місто та номер відділення Нової Пошти.\n"
        "2. **Кур'єрська доставка:** Цей режим активується, ТІЛЬКИ ЯКЩО клієнт прямо написав ключові слова: «кур'єр», «адресна доставка», «доставка на адресу». У цьому випадку, тобі більше НЕ ПОТРІБЕН номер відділення. Твоя задача — отримати від клієнта повну адресу (місто, вулиця, будинок). Коли ти формуєш JSON, ти повинен покласти всю цю адресу в поле `city`, а поле `np` залишити порожнім (`\"\"`).\n\n"
        ...
        '  "phone": "0XX-XXXX-XXX",\n'
        '  "city": "Місто АБО повна адреса для кур\'єра",\n'
        '  "np": "Номер відділення (або "" для кур\'єра)",\n'
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if awaiting == {4}:
        ...
        # --- ПОЧАТОК ЗМІН ---
        # Дозволяємо замовлення без 'np' для кур'єрської доставки
