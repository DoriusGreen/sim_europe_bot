...
def _order_signature(order: OrderData) -> str:
    ...
    return f"{format_full_name(order.full_name)}|{format_phone(order.phone)}|{order.city.strip()}|{str(order.np or '').strip()}|{items_sig}"
...
def render_order(order: OrderData) -> str:
    ...
    if order.city and order.np:
        ...
        delivery_line = f"{format_city(order.city)} № {format_np(order.np)}"
    elif order.city:
...
def build_system_prompt() -> str:
    return (
        ...
        "2. **Стандартна доставка:** В усіх інших випадках доставка йде на відділення «Нової Пошти».\n"
        "3. **СУВОРА ЗАБОРОНА:** Якщо в повідомленні НЕМАЄ ключових слів для кур'єрської доставки, то поле `np` (номер відділення) **ОБОВ'ЯЗКОВО** повинно бути заповнене. Якщо ти не можеш знайти номер відділення в тексті (наприклад, клієнт вказав тільки вулицю), **ЗАБОРОНЕНО** заповнювати поле `city` цією адресою. В такому випадку ЗАЛИШ ОБИДВА ПОЛЯ, `city` та `np`, ПОРОЖНІМИ (`\"\"`). Це критично важливо, щоб бот міг перепитати клієнта.\n\n"
        # --- КІНЕЦЬ ЗМІН ---
...
def build_manager_parser_prompt() -> str:
    ...
    return (
        ...
        "- Номер телефону.\n"
        "- Місто та номер відділення або поштомату «Нової Пошти», або повну адресу для кур'єрської доставки.\n"
        ...
        # --- ПОЧАТОК ЗМІН ---
        "4. **Правило доставки:** Тільки якщо в тексті є явні ключові слова (напр. «кур'єр», «адресна»), трактуй доставку як кур'єрську. В такому випадку записуй всю адресу в поле `\"city\"`, а поле `\"np\"` залишай порожнім. В іншому випадку, якщо номер відділення не вказано чітко, залишай `city` та `np` порожніми.\n"
        # --- КІНЕЦЬ ЗМІН ---
