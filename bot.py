...
def build_system_prompt() -> str:
    return (
        ...
        "Пункт 4 може бути у довільній формі/порядку («Англія 2 шт», «2 штуки англія», «дві UK», «UK x2», «по Англії дві» тощо).\n"
        ...
        # === ДОСТУПНІ КРАЇНИ (СУВОРО БІЛИЙ СПИСОК) ===
...
def build_followup_prompt() -> str:
    return (
        "Прайс щойно надіслано окремим повідомленням. "
        "Відповідай КОРОТКО на інші частини останнього повідомлення, що НЕ стосуються цін/прайсу. "
        "Не повторюй ціни, не надсилай JSON.\n\n"
        "ВАЖЛИВО: якщо користувач просив ЛИШЕ ціну/вартість/прайс і нічого більше — поверни порожній рядок. "
        "НЕ пиши підтвердження наявності та подібні фрази (на кшталт «підтверджую наявність», «є в наявності», «available»).\n\n"
        "Якщо питали про терміни відправки — «Відправляємо протягом 24 годин.»; "
        "якщо про ТТН/трек-номер/чи вже відправили — "
        "«Зазвичай відправляємо в день замовлення або протягом 24 годин. Якщо потрібна ТТН — очікуйте відповіді менеджера.» "
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if price_countries is not None:
        ...
        # 3b) Фоллоу-ап (інші частини питання); якщо порожній/незмістовний — нічого не слати
    ...
    # 4) Якщо бракує лише пункту 4 — пробуємо «force point 4»
